...
| OFFICIAL_DRIVERS | **OPEN** | EXECUTING, not complete (R6-QUAL-01, 2026-08-20). All six official-driver rows run: 1,132 behaviour scenarios across Rust/Python/TypeScript on fork-classic (U1) and fork-slatedb (U2), 1,102 in-plan leaf outcomes, 0 failures, each bundle independently re-derived from its archived bytes by tools/evidence/verify_drivers.py and 99/99 mutants killed. The SlateDB backend passes the full official suites of all three drivers identically to RocksDB, evidenced by the archived on-disk witness _system/backend-spec.marker (kind slatedb-r2 / object-store local-fs vs kind classic) rather than by an environment-variable claim. The lane stays OPEN because three preconditions are unmet, each named with its blocker in docs/evidence/G1/drivers/driver-row-status.json: cluster suites need a 3-node replicated deployment plus test certificates (TypeDB CE is single-node); the TypeScript Bazel type surface is not reproducible outside Bazel (upstream emits no index.d.cts, its own BUILD TODO); and the pristine/U0 lane is disk-blocked, measured. Plan rows remain PARTIAL, never COVERED. A further denominator gap is recorded and NOT closed: driver/migration.feature and driver/cluster.feature have no plan leaves at all, because generate_catalog.py only enumerates feature files referenced from sources/typedb, so 18 executed migration scenarios cover nothing and are recorded as leaves_outside_plan. |
