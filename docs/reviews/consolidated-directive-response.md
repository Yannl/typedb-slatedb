...
| **Q-14** cache removal errors ignored | **OPEN** | Not addressed. |
...
| **Q-27** `NoDeleteStore` composite mutators | **OPEN** | Not addressed: rename/copy/multipart paths are still unaudited against the wrapper. |
...
| **Q-12** admission bounds | **PARTIAL** `7d70306` | The payload route buffered the whole body before consulting any budget, so the first oversized object was always admitted and base64-expanded in a 128 MiB isolate — now a declared-length check before the body is read, with a post-read backstop. Scan pages no longer exempt "record zero". Remainder OPEN: mandatory budget rows before generation activation, transactional usage counters, streaming/backpressure framing, and the server-owned opaque `SnapshotId` (the pinned iterator still accepts a caller-supplied `throughLsn`). |
...
| **Q-17** SQL cost grows with history | **OPEN** | The append path still does `COUNT(*)` over `wal_tail` and `control_outbox` per finalisation. Not addressed. |
...
| **§12.6** `/wal/finalize-batch` without the v16 batch schema | **OPEN** | Accepted and **not** done: the route is still deployable without `BatchOperationId`, a canonical batch digest, or K/byte limits. The directive's minimum is to make it physically test-only before any external deployment; neither that nor the full envelope was built. |
