...
| **A5** | Authoritative counters are JS `number`; `generation` unguarded | **CLOSED** (`997da46` + `0953ec0`): AppendLsn/TypeSequence/ControlSeq are exact u64 (BE-blob SQL, bigint core, decimal-string wire) with a 2⁵³ mutant control; `generation` is now guarded at every wire entry point — all seven /wal path routes, both checkpoint-cut routes, register/fence bodies, finalize/finalize-batch bodies — as an exact non-negative integer (typed 400 `INVALID_GENERATION`, never a rounded coercion). E2E controls: a 30-digit path generation and a `1e300` body generation both refuse typed. |
...
| **A10** | `get_prev` panics the whole process on any transient object-store error | **CLOSED** (`5d05d5c`). SlateDB's own `ErrorKind::Unavailable` is the contractual transient class; `get_prev` retries it up to 4 times with backoff before the fail-closed panic, and every other kind (Invalid/Data/Internal/Closed/Transaction) still fails closed immediately — retrying those would mask corruption behind latency. The decision is a named helper with three controls; the retry-everything mutant was executed and caught. This is the reimplementation of the donor's `RetryClass` intent against this branch's layout. |
