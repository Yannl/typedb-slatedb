...
pub(crate) struct Keyspace {
    ...
    name: &'static str,
    id: KeyspaceId,
    // shared with `RawCursor`s handed out via the `IteratorPool`, which co-own
    // the database so that no cursor can ever observe a closed `DB`
    pub(super) kv_storage: Arc<DB>,
