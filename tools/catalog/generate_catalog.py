    ...
    for ref in sorted(refs):
        ...
        while i < len(lines):
            ...
            if line.startswith("Scenario:"):
                scenarios.append({"name": line.split(":", 1)[1].strip(), "outline_examples": None})
            i += 1
...
def main():
    ...
    print(
        f"cargo targets: {len(cargo_targets)} total, "
    ...
    profiles = [
        {
            "profile_id": "U0",
            "kv_backend": "rocksdb-pristine",
            "durability": "file-wal",
            "object_store": "none",
            "controller": "upstream-local",
            "features": [],
            "cfg": [],
        },
        {
            "profile_id": "U1",
            "kv_backend": "rocksdb-adapter",
            "durability": "file-wal",
            "object_store": "none",
            "controller": "fork-local",
            "features": [],
            "cfg": [],
        },
        {
            "profile_id": "U2",
            "kv_backend": "slatedb-localfs",
            "durability": "file-wal",
        ...
        },
        {
            "profile_id": "U3",
            "kv_backend": "slatedb-localfs",
            "durability": "remote-wal",
            "object_store": "localfs",
            "controller": "deterministic-model",
            "features": [],
            "cfg": [],
        },
        {
            "profile_id": "U4",
            "kv_backend": "slatedb-r2",
            "durability": "remote-wal",
            "object_store": "r2",
            "controller": "real-do",
            "features": [],
    ...
    for t in sorted(cargo_targets, key=lambda x: (x["package"], x["target_name"], x["kind"])):
        ...
        case_discovery = (
            "CUCUMBER_SCENARIOS"
            if is_behaviour
            else "FAILPOINT_REGISTRY"
            if t["target_name"] == "test_fail_points"
            else "LIBTEST_LIST"
        )
        env = {}
        fixture_ids = []
        if is_assembly:
            env["TYPEDB_ASSEMBLY_ARCHIVE"] = (
                "<staged by runner: cargo-built typedb-all-linux-x86_64 archive>"
            )
            fixture_ids = [
                "fixture:typedb-console-linux-x86_64-3.12.0",
    ...
    kind_by_target = {
        (t["package"], t["target_name"]): (t["kind"], t["src_sha256"]) for t in cargo_targets
    }
    for (pkg, tname), cs in sorted(lib_cases.items()):
        kind, src_hash = kind_by_target.get((pkg, tname), ("test", "0" * 64))
        tid = f"cargo:{pkg}:unit:{tname}" if kind == "unit" else f"cargo:{pkg}:{kind}:{tname}"
    ...
    targets.append(
        {
            ...
            "cargo_target": None,
            "source_files": [
                {"path": "tool/test/simulate-crash.sh", "sha256": sha256_file(crash_sh)}
            ],
    ...
    catalog = {
        ...
        "rust_toolchain": {
            "rustc": subprocess.check_output(
                ["rustc", TOOLCHAIN, "--version"], env=ENV, text=True
            ).strip(),
            "cargo": subprocess.check_output(
                ["cargo", TOOLCHAIN, "--version"], env=ENV, text=True
            ).strip(),
        },
    ...
    summary = {
        ...
        "libtest_cases": sum(1 for leaf in leaf_cases if leaf["kind"] == "LIBTEST"),
        "cucumber_cases": sum(1 for leaf in leaf_cases if leaf["kind"] == "CUCUMBER"),
        "failpoint_cases": sum(1 for leaf in leaf_cases if leaf["kind"] == "FAILPOINT"),
        "script_cases": sum(1 for leaf in leaf_cases if leaf["kind"] == "SCRIPT"),
        "static_checks": sum(1 for leaf in leaf_cases if leaf["kind"] == "STATIC_CHECK"),
        "leaf_cases_total": len(leaf_cases),
        "declared_ignored": sum(1 for leaf in leaf_cases if leaf["declared_ignored"]),
