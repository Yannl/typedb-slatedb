...
def judge(name, res, out_dir, expect_substring):
    ...
    return {
        ...
        "message_excerpt": next(
            (line for line in text.splitlines() if expect_substring in line),
            text.strip().splitlines()[-1] if text.strip() else "",
        ),
        "outcome": "KILLED" if killed and expect_substring in text else "SURVIVED",
        "argv": res["argv"],
    }
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="live-mutants-") as td:
        ...
        r = invoke(
            out1, args.run_dir / "m1", ["--server-binary", str(td / "no-such-typedb-server")]
        )
        results.append(judge("server_binary_missing", r, out1, "server binary absent"))
        ...
        try:
            ...
            results.append(judge("server_port_occupied", r, out2, "already bound"))
        except OSError as e:
            results.append(
                {
                    "mutant": "server_port_occupied",
                    "outcome": "SKIPPED",
                    "reason": f"could not bind 1729 to stage the mutant: {e}",
                }
            )
        finally:
            s.close()
        ...
        r = invoke(
            out3, args.run_dir / "m3", ["--skip-build", "--target-dir", str(td / "empty-target")]
        )
        results.append(judge("suite_never_built", r, out3, "cargo produced no test executable"))
    ...
    doc = {
        "schema": "driver-lane-live-mutants-v1",
        "statement": (
            "Execution-time negative controls: the runner itself "
    ...
    print(f"LIVE MUTANTS: {killed} killed, {survived} survived, {skipped} skipped", file=sys.stderr)
