...
def main() -> int:
    ...
    try:
        ...
        dangling = subprocess.run(
            ["git", "-C", str(wt), "commit-tree", "HEAD^{tree}",
             "-m", "ledger-mutants: dangling non-ancestor"],
            capture_output=True, text=True, check=True,
            env={**os.environ, **GIT_IDENT}).stdout.strip()
        ...
        MUTANTS = [
            ("duplicate-gate-id",
             lambda l: l["gates"].append(dict(l["gates"][0])),
             "duplicate gate id"),
            ("duplicate-lane-id",
             lambda l: l["lanes"].append(dict(l["lanes"][0])),
             "duplicate lane id"),
            ("duplicate-action-id",
             lambda l: l["actions"].append(dict(l["actions"][0])),
             "duplicate action id"),
            ("gate-state-outside-enum",
             lambda l: gate(l, "G0").__setitem__("state", "CLOSED_GREEN"),
             "not a recognised gate state"),
            ("lane-state-outside-enum",
             lambda l: l["lanes"][0].__setitem__("state", "GREEN"),
             "not in the lane-state enum"),
            ("action-status-outside-enum",
             lambda l: action(l, "PR0").__setitem__("status", "FINISHED"),
             "not in the action-status enum"),
            ("done-action-with-no-commits",
             lambda l: action(l, "R4-A").__setitem__("commits", []),
             "a closed action must cite the commits that closed it"),
            ("done-action-citing-nonexistent-commit",
             lambda l: action(l, "R4-A").__setitem__(
                 "commits", ["1234567890abcdef1234567890abcdef12345678"]),
             "does not exist in this repository"),
            ("done-action-citing-non-ancestor-commit",
             lambda l: action(l, "R4-A").__setitem__("commits", [dangling]),
            ...
            ("gate-blocking-finding-closed-by-done-action",
             ...
             lambda l: gate(l, "G1")["blocking_findings"].append("C-P0-01"),
             "records it closed"),
            ("closes-claim-on-unclosed-action",
             lambda l: action(l, "PR6").__setitem__("closes", ["R4-CF-00"]),
             "not a closed status"),
            ("closed-action-silently-reopened",
             lambda l: action(l, "R4-A").__setitem__("status", "OPEN"),
             "impossible status transition"),
            ("action-row-erased-from-history",
             lambda l: l["actions"].__setitem__(
                 slice(None), [a for a in l["actions"] if a["id"] != "PR0"]),
             "history rows may be corrected, never erased"),
            ("ledger-edit-without-rerender",
             lambda l: gate(l, "G1").__setitem__(
                 "why", gate(l, "G1")["why"] + " (mutated)"),
             "gate table drifted from the ledger"),
            ("ledger-references-missing-evidence-path",
             lambda l: l["q_dispositions"].__setitem__(
        ...
        (wt / LEDGER_REL).write_text(pristine_ledger)
        (wt / "docs" / "operations.md").write_text(
        ...
        if rc == 0:
            ...
            failures.append(
                f"mutant 'forbidden-claim-in-status-doc' died for the WRONG "
                f"reason:\n{out}")
    finally:
    ...
    print(f"LEDGER MUTANTS: {verdict} "
