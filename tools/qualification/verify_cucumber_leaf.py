...
def verify(out_dir, plan=None, catalog=None, repo=REPO, corpus=None):
    ...
    if bundle.get("plan_root") != plan.get("plan_root"):
        A.append(f"bundle pins plan_root {bundle.get('plan_root')} but the plan "
    ...
    if bundle.get("catalog_sha256") != cat_sha:
        A.append(f"bundle pins catalog_sha256 {bundle.get('catalog_sha256')} but "
                 f"the catalogue now hashes {cat_sha}")
    prof = bundle.get("profile")
    if bundle.get("profile_in_plan") != (prof in plan["profiles"]):
        A.append(f"bundle claims profile_in_plan={bundle.get('profile_in_plan')} "
    ...
    if want and bh.get("checkout_revision") != want:
        A.append(f"the behaviour corpus was at revision "
                 f"{bh.get('checkout_revision')} but the plan declares {want}")
    if bh.get("checkout_dirty"):
        A.append("the behaviour corpus checkout was DIRTY - the feature bytes "
    ...
    for s in bundle.get("sources", []):
        ...
        if anomalies and claimed_clean:
            A.append(f"source bundle {s['bundle']} is presented as usable but "
        ...
        if head != f"COMPLETE {facts['bundle_root']}":
            if claimed_clean:
                A.append(f"source bundle {s['bundle']} carries COMPLETE {head!r} "
        ...
        if s.get("profile") != sb.get("profile"):
            A.append(f"source bundle {s['bundle']} records profile "
                     f"{sb.get('profile')!r}, the row says {s.get('profile')!r}")
        if sb.get("profile") != prof and claimed_clean:
            A.append(f"source bundle {s['bundle']} is profile "
    ...
    if tcids and bundle.get("toolchain_id") not in tcids:
        A.append(f"bundle records toolchain_id {bundle.get('toolchain_id')!r} "
    ...
    for s in bundle.get("sources", []):
        ...
        for lg in s.get("logs", []):
            ...
            if sb is not None and src_row is None:
                A.append(f"{raw}: the bundle reads a log the sealed source "
            ...
            if actual != lg.get("log_sha256"):
                A.append(f"{raw}: hashes {actual} but the row records "
                         f"{lg.get('log_sha256')}")
                continue
            if src_row is not None and actual != src_row.get("log_sha256"):
                A.append(f"{raw}: hashes {actual} but the SEALED source bundle "
            ...
            try:
                ...
                inside = False
            if not inside:
                A.append(f"{raw}: lives outside its source bundle "
            ...
            if not_ok:
                bad.append(f"{len(not_ok)} cucumber summary block(s) are not "
                           f"all-passed")
            summed = sum(x["scenarios"] or 0 for x in summaries)
            if summed != len(scanned):
                bad.append(f"summaries count {summed} scenario(s), "
                           f"{len(scanned)} scanned")
            for x in summaries:
                if x["features"] != x["scenarios"]:
                    bad.append(f"[Summary] at line {x['line']}: "
                               f"{x['features']} features vs {x['scenarios']} "
                               f"scenarios")
            if lg.get("publishable") and bad:
                A.append(f"{raw}: marked publishable but re-derivation refuses "
                         f"it: {bad[0]}")
            if not lg.get("publishable") and not bad:
                A.append(f"{raw}: marked NOT publishable while re-derivation "
            ...
            if lg.get("scanned_scenarios") != len(scanned):
                A.append(f"{raw}: row claims {lg.get('scanned_scenarios')} "
                         f"scanned scenario(s), the bytes yield {len(scanned)}")
            if lg.get("summary_blocks") != len(summaries):
                A.append(f"{raw}: row claims {lg.get('summary_blocks')} summary "
                         f"block(s), the bytes carry {len(summaries)}")
            plain, outline = cc.count_keyword_lines(text)
            if (lg.get("keyword_lines") or {}) != {"scenario": plain,
                                                   "scenario_outline": outline}:
                A.append(f"{raw}: row claims keyword line counts "
            ...
            if runner != lg.get("runner"):
                A.append(f"{raw}: row calls the runner {lg.get('runner')!r} but "
            ...
            scan_cache[raw] = {"per_title": per_title, "all_passed": not not_ok,
                               "scenarios": len(scanned), "text_lines": None}
        ...
        for lg in s.get("logs", []):
            ...
            for title, items in sc["per_title"].items():
                ...
                obs_by_target[rec["target_id"]].append(
                    {"bundle": s["bundle"], "raw_log": lg["raw_log"],
                     "cargo_target": lg["cargo_target"], "runner": lg["runner"],
                     "items": items})
    ...
    for tid, obs in obs_by_target.items():
        ...
        if not sound:
            continue
        sound.sort(key=lambda o: (-len(o["run"]),
        ...
        if own.get("raw_log") != sound[0]["raw_log"]:
            A.append(f"{rec['ref']}: the bundle names {own.get('raw_log')!r} as "
        ...
        claimed_t = {(t["template"], t["declaration_line"]): t
                     for t in (cf.get("templates") or [])}
        for k, v in per_t.items():
            ct = claimed_t.get(k)
            if ct is None:
                A.append(f"{rec['ref']}: the bundle records no per-template "
                         f"reconciliation for outline {k[0]!r} at line {k[1]}")
                continue
            if ct.get("catalogued_examples") != v["catalogued"] \
                    or ct.get("runtime_matched") != v["runnable"] \
                    or ct.get("runnable_examples") != v["runnable"]:
                A.append(f"{rec['ref']}: template {k[0]!r} at line {k[1]} - the "
            ...
            if idx != sorted(idx):
                A.append(f"{rec['ref']}: template {k[0]!r} binds example indices "
    ...
    for l in catalog["leaf_cases"]:
        if l["kind"] == "CUCUMBER":
            cat_by_target[l["target_id"]][l["leaf_case_id"]] = l
    entry_by_id = {}
    ...
    for lf in leaves:
        ...
        if pair is None:
            A.append(f"{lid}: no catalogued cucumber scenario re-expands to this "
                     f"leaf id - a leaf bound to nothing is refused")
            continue
        rec, e = pair
        if rec["target_id"] != tid:
            A.append(f"{lid}: bound to target {tid!r} but the catalogue places "
                     f"it under {rec['target_id']!r}")
        if cat_by_target.get(tid, {}).get(lid, {}).get("display_name") \
                != lf.get("display_name"):
            A.append(f"{lid}: row says display_name {lf.get('display_name')!r}, "
                     f"the catalogue says "
                     f"{cat_by_target.get(tid, {}).get(lid, {}).get('display_name')!r}")
        if lf.get("runtime_name") != e["runtime_name"]:
            A.append(f"{lid}: row says the runtime printed "
                     f"{lf.get('runtime_name')!r}, the expansion of example "
                     f"{e['example_index']} yields {e['runtime_name']!r}")
        if lf.get("anchor") != e["anchor"] \
                or (plan.get("leaves") or {}).get(lid, {}).get("anchor") != e["anchor"]:
            A.append(f"{lid}: anchor {lf.get('anchor')!r} does not agree with the "
                     f"expansion ({e['anchor']!r}) and the plan "
                     f"({(plan.get('leaves') or {}).get(lid, {}).get('anchor')!r})")
        if lf.get("example_index") != e["example_index"] \
                or lf.get("example_total") != e["example_total"]:
            A.append(f"{lid}: row says example {lf.get('example_index')}/"
        ...
        if lf.get("fixture_set_id") != fs_id:
            A.append(f"{lid}: row files the leaf under fixture set "
                     f"{lf.get('fixture_set_id')!r}, the plan says {fs_id!r}")
        raw = lf.get("raw_log")
        sc = scan_cache.get(raw)
        if sc is None:
            A.append(f"{lid}: names log {raw} which this bundle does not read "
                     f"or which was refused")
            continue
        if lf.get("outcome") != "PASSED" or not sc["all_passed"]:
            A.append(f"{lid}: claims outcome {lf.get('outcome')!r} from log {raw}, "
        ...
        if not isinstance(n, int) or not (1 <= n <= len(lines)) \
                or not isinstance(col, int) or col < 1:
            A.append(f"{lid}: log_line/log_column {n!r}/{col!r} are outside its "
        ...
        if got != want:
            ...
            A.append(f"{lid}: line {n} column {col} of {raw} diverges from the "
        ...
        if len(pos) != 1:
            A.append(f"{lid}: line {n}/{col} of {raw} is not exactly one of that "
        ...
        if run is not None and raw == (exp_owner[tid]["raw_log"]):
            if pos[0] >= len(run) or run[pos[0]]["leaf_case_id"] != lid:
                A.append(f"{lid}: is the {pos[0]+1}th scenario line of "
    ...
    for tid, owner in exp_owner.items():
        ...
        if per_target.get(tid, 0) != want:
            A.append(f"{rec['ref']}: publishes {per_target.get(tid, 0)} leaf/leaves "
                     f"but the owning log {owner['raw_log']} executed {want} "
                     f"scenario(s)")
        got_ids = {lid for _p, lid in ordinal_seen.get((tid, owner["raw_log"]), [])}
        missing = [e["leaf_case_id"] for e in owner["run"]
                   if e["leaf_case_id"] not in got_ids]
        if missing:
            A.append(f"{rec['ref']}: {len(missing)} executed scenario(s) have no "
    ...
    for tid, owner in exp_owner.items():
        ...
        for e in owner["skipped"]:
            if e["leaf_case_id"] in seen:
                A.append(f"{e['leaf_case_id']}: the {owner['runner']} runner's "
                         f"tag filter excluded it, yet the bundle publishes an "
                         f"outcome for it")
            if e["leaf_case_id"] not in claimed_not_run:
                A.append(f"{e['leaf_case_id']}: excluded by the runner's tag "
    ...
    for k, v in want_jr.items():
        if got_jr.get(k) != v:
            A.append(f"join_reconciliation.{k} is {got_jr.get(k)!r} in the "
    ...
    if mf.is_file():
        ...
        if m.get("bundle_root") != root:
            A.append(f"sidecar manifest root {m.get('bundle_root')} != "
                     f"recomputed {root}")
        for rel, sha in (m.get("files") or {}).items():
            if pairs.get(rel) != sha:
                A.append(f"sidecar manifest binds {rel}={sha} but it now hashes "
                         f"{pairs.get(rel)}")
        for rel in pairs:
            if rel not in (m.get("files") or {}):
                A.append(f"the bundle consumes {rel} which the sidecar manifest "
                         f"does not bind")
    else:
    ...
    outcomes = collections.Counter(l.get("outcome") for l in leaves)
    facts = {
        "bundle": str(out_dir.relative_to(repo)) if out_dir.is_relative_to(repo)
                  else str(out_dir),
        "profile": prof,
        "toolchain_id": bundle.get("toolchain_id"),
        "features_published": sum(1 for f in (bundle.get("features") or [])
                                  if f.get("leaves_published")),
        "leaves": len(leaves),
...
def main():
    ...
    for d in args.dirs:
        ...
        if A:
            ...
            continue
        if args.seal:
            (pathlib.Path(d) / "COMPLETE").write_text(
                f"COMPLETE {facts['bundle_root']}\n")
            print(f"SEALED {d} COMPLETE {facts['bundle_root']}", file=sys.stderr)
    print(f"CUCUMBER LEAF BUNDLE VERIFY: {'CLEAN' if rc == 0 else 'REFUSED'}",
          file=sys.stderr)
    return rc
