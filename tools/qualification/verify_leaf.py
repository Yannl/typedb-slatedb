...
def verify(out_dir, plan=None, catalog_leaves=None, catalog_targets=None,
    ...
    if state not in ("PRISTINE", "FORK_STAGED_EXACT"):
        A.append(f"executed tree_state is {state!r} - only PRISTINE or "
    ...
    if state == "PRISTINE" and (tree.get("dirty") or tree.get("staged_delta_files")):
        A.append(f"bundle claims a PRISTINE tree while recording "
    ...
    if mf.is_file():
        ...
        if m.get("bundle_root") != root:
            A.append(f"sidecar manifest root {m.get('bundle_root')} != recomputed "
                     f"{root}")
    ...
    if marker.is_file():
        ...
        if not m:
            ...
            A.append(f"COMPLETE binds root {m.group(1)} but the bundle recomputes "
    ...
    for r in rows:
        ...
        raw = r.get("raw_log")
        if not raw:
            A.append(f"{rid}: records no raw log - counts without a log are an "
        ...
        key = log.resolve()
        if key in seen_log:
            A.append(f"{rid} and {seen_log[key]} both name log {raw} - one "
                     f"execution cannot vouch for two rows")
            continue
        seen_log[key] = rid
        actual = common.sha256_file(log)
        if actual != r.get("log_sha256"):
            A.append(f"{rid}: log {raw} hashes {actual} but the row recorded "
        ...
        if r.get("publishable"):
            ...
            if declared and not cases:
                A.append(f"{rid}: marked publishable with ZERO parsed cases while "
            ...
            if ctid is not None and rid_map.get(rid) != ctid:
                A.append(f"{rid}: bound to catalogue target {ctid!r} but the shared "
    ...
    for lf in leaves:
        ...
        if lid in seen_leaf:
            ...
            continue
        seen_leaf.add(lid)
        rid, ctid, name = (lf.get("runner_row_id"), lf.get("catalog_target_id"),
                           lf.get("case_name"))
        per_target[rid] = per_target.get(rid, 0) + 1
        if rid not in target_by_rid:
            A.append(f"{lid}: bound to target row {rid!r} which this bundle does "
                     f"not contain - a leaf from a target that never ran")
            continue
        if lid != f"{ctid}::{name}":
            A.append(f"{lid}: id is not <catalog_target_id>::<case_name> "
                     f"({ctid!r}, {name!r})")
        declared = catalog_leaves.get(ctid, {})
        if name not in declared:
            A.append(f"{lid}: the catalogue declares no leaf {name!r} under target "
                     f"{ctid!r} - a leaf bound to the wrong target_id is refused, "
                     f"never repaired")
        elif declared[name]["leaf_case_id"] != lid:
            A.append(f"{lid}: catalogue leaf id for {name!r} under {ctid!r} is "
                     f"{declared[name]['leaf_case_id']}")
        if rid_map.get(rid) != ctid:
            A.append(f"{lid}: runner row id {rid!r} joins to catalogue target "
                     f"{rid_map.get(rid)!r}, not {ctid!r}")
        tr = target_by_rid.get(rid) or {}
        if lf.get("log_sha256") != tr.get("log_sha256"):
            A.append(f"{lid}: binds log sha {lf.get('log_sha256')} but its target "
    ...
    for rid, r in target_by_rid.items():
        if not r.get("publishable"):
            if per_target.get(rid):
                A.append(f"{rid}: refused target published {per_target[rid]} "
                         f"leaf/leaves anyway")
            continue
        ctid = r.get("catalog_target_id")
        declared = catalog_leaves.get(ctid, {})
        expect = r.get("parsed_cases", 0) - len(r.get("extra_cases") or [])
        if per_target.get(rid, 0) != expect:
            A.append(f"{rid}: publishes {per_target.get(rid, 0)} leaf/leaves but "
        ...
        if declared and per_target.get(rid, 0) == 0:
            A.append(f"{rid}: publishable and catalogued case-bearing "
