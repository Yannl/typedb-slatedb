...
def main():
    ...
    ap.add_argument("--source", default=None, help="a promoted release directory to mutate")
    args = ap.parse_args()

    source = args.source
    if source is None:
        candidates = [
            d for d in sorted(DEFAULT_RELEASES.glob("*")) if (d / "promotion.json").exists()
        ]
        if not candidates:
            print("no promoted release found; run build/test/promote first", file=sys.stderr)
            return 2
        source = candidates[-1]
    s = Suite(source)
    ...
    with s.tampered("control") as releases:
        ...
        if code != 0:
            print(
                f"CONTROL FAILED: an untampered copy did not verify (exit={code})\n{text}",
                file=sys.stderr,
            )
            return 2
    ...
    def make_lane_fail(r, lane, verdict_word):
        ...
        log.write_text(log.read_text().replace(f"{verdict_word}: PASS", f"{verdict_word}: FAIL"))
        digest_now = rel.sha256_file(log)
        edit_json(
            r / d / f"lanes/{lane}.json",
            lambda doc: doc.update(passed=False, attempt=1, log_sha256=digest_now),
        )
        (r / d / f"lanes/{lane}.attempts.jsonl").write_text(
            json.dumps(
                {"attempt": 1, "artifact_sha256": d, "passed": False, "log_sha256": digest_now},
                sort_keys=True,
                separators=(",", ":"),
            )
            + "\n"
    ...
    with s.tampered("lane-json-flip") as r:
        ...
        edit_json(r / d / "lanes/artifact-exec.json", lambda doc: doc.update(passed=True))
        (r / d / "lanes/artifact-exec.attempts.jsonl").write_text(
            json.dumps(
                {"attempt": 1, "artifact_sha256": d, "passed": True, "log_sha256": log_digest},
                sort_keys=True,
                separators=(",", ":"),
            )
            + "\n"
        ...
        s.expect(
            "failing lane laundered by editing its JSON verdict",
            ["--releases", str(r), "promote", "--digest", d, "--key-id", "dev-public"],
            "LANE_VERDICT_CONTRADICTION",
        )
    ...
    with s.tampered("lane-log-edit-rehash") as r:
        ...
        edit_json(
            r / d / "lanes/artifact-layout.json",
    ...
    with s.tampered("lane-missing") as r:
        ...
        s.expect("required lane result deleted", s.verify_argv(r), "REQUIRED_LANE_MISSING")

    with s.tampered("lane-foreign-artifact") as r:
        edit_json(
            r / d / "lanes/artifact-exec.json", lambda doc: doc.update(artifact_sha256="0" * 64)
        )
        s.expect("lane bound to another artifact", s.verify_argv(r), "LANE_BOUND_TO_OTHER_ARTIFACT")

    with s.tampered("lane-rebuilt-during") as r:
        edit_json(
            r / d / "lanes/artifact-exec.json",
            lambda doc: doc.update(artifact_sha256_after="0" * 64),
        )
        s.expect(
            "artifact changed during the lane", s.verify_argv(r), "LANE_BOUND_TO_OTHER_ARTIFACT"
        )

    # --- attempt ledger --------------------------------------------------
    with s.tampered("attempts-removed") as r:
        (r / d / "lanes/artifact-exec.attempts.jsonl").unlink()
        s.expect("attempt ledger deleted", s.verify_argv(r), "LANE_ATTEMPTS_MISSING")
    ...
    def add_retry(r, lane):
        ...
        (r / d / f"lanes/{lane}.attempts.jsonl").write_text(
            "".join(
                json.dumps(
                    {"attempt": i, "artifact_sha256": d, "passed": True, "log_sha256": log_digest},
                    sort_keys=True,
                    separators=(",", ":"),
                )
                + "\n"
    ...
    with s.tampered("attempts-truncated") as r:
        ...
        s.expect(
            "retry hidden by truncating the attempt ledger",
            ["--releases", str(r), "promote", "--digest", d, "--key-id", "dev-public"],
            "LANE_RESULT_NOT_LAST_ATTEMPT",
        )
    ...
    with s.tampered("attempts-retry-visible") as r:
        ...
        (r / d / "signature.json").unlink()
        add_retry(r, "artifact-layout")
        code, text = run(["--releases", str(r), "promote", "--digest", d, "--key-id", "dev-public"])
        if code != 0:
            print(f"SURVIVED honest retry could not promote at all: {text[:300]}")
            s.survived.append(("retried lane promotes but is flagged", code, text[:300]))
        else:
            s.expect(
                "retried lane is flagged, not hidden",
    ...
    with s.tampered("junk-file") as r:
        ...
        s.expect(
            "unaccounted file added to the release directory", s.verify_argv(r), "ROOT_MISMATCH"
        )

    with s.tampered("record-edited") as r:
        edit_json(r / d / "release.json", lambda doc: doc["artifact"].update(member_root="f" * 64))
        s.expect("release record edited after signing", s.verify_argv(r), "ROOT_MISMATCH")

    with s.tampered("promotion-edited") as r:
        edit_json(r / d / "promotion.json", lambda doc: doc.update(promotion_class="RELEASE"))
        s.expect("promotion class upgraded after signing", s.verify_argv(r), "ROOT_MISMATCH")
    ...
    with s.tampered("signature-forged") as r:
        ...
        edit_json(r / d / "signature.json", lambda doc: doc.update(signature="ab" * 64))
    ...
    with s.tampered("signature-other-key") as r:
        ...
        s.expect("valid signature by an untrusted key", s.verify_argv(r), "SIGNING_KEY_SUBSTITUTED")

    with s.tampered("signature-unknown-key-id") as r:
        edit_json(r / d / "signature.json", lambda doc: doc.update(key_id="not-a-registered-key"))
        s.expect(
            "signature claiming an unregistered key id", s.verify_argv(r), "UNTRUSTED_SIGNING_KEY"
        )
    ...
    with s.tampered("signature-second-opinion") as r:
        ...
        try:
            s.expect(
                "independent verifier disagrees with the Python one",
                s.verify_argv(r),
                "SIGNATURE_IMPLEMENTATION_DISAGREEMENT",
            )
        finally:
    ...
    with s.tampered("artifact-substituted") as r:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            s.expect(
                "a different artifact presented under this digest",
    ...
    with s.tampered("promote-failing-lane") as r:
        ...
        s.expect(
            "promotion attempted with a failing required lane",
            ["--releases", str(r), "promote", "--digest", d, "--key-id", "dev-public"],
    ...
    with s.tampered("promote-unknown-key") as r:
        ...
        (r / d / "signature.json").unlink()
        s.expect(
            "promotion with an unregistered signing key",
            ["--releases", str(r), "promote", "--digest", d, "--key-id", "attacker-key"],
            "UNTRUSTED_SIGNING_KEY",
        )

    with s.tampered("release-grade-development") as r:
        s.expect(
            "development-class promotion claimed as release grade",
            s.verify_argv(r, ("--release-grade",)),
    ...
    with s.tampered("record-immutable") as r:
        ...
        s.expect(
            "rebuilding over an existing record with new content",
            ["--releases", str(r), "build"],
            "RELEASE_RECORD_IMMUTABLE",
            expect_exit=(2,),
        )
    ...
    with s.tampered("lane-not-implemented") as r:
        s.expect(
            "a required lane that names no implementation",
