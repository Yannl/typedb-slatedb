...
pub fn spawn_issuer(mode: &str, environment: &str, tenant: &str, bearer: &str) -> IssuerSidecar {
    ...
    let line = rx
        ...
        .expect("issuer sidecar announced nothing within 60s");
    ...
    assert!(
        ...
        "issuer sidecar failed to start: {line}"
    );
...
pub fn spawn_wrangler(config: &str, port: u16, vars: &[(String, String)]) -> ProcGroup {
    ...
    let mut args: Vec<String> = vec![
        "wrangler".into(), "dev".into(), "--local".into(),
        "-c".into(), config.into(),
        "--port".into(), port.to_string(),
        "--persist-to".into(), persist_to.to_string_lossy().into_owned(),
    ];
